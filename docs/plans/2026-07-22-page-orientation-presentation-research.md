...
**Architecture:** Both Firefox and Chromium arrive at NVDA's shared `BrowseModeDocumentTreeInterceptor.event_treeInterceptor_gainFocus` presentation method after backend-specific virtual-buffer selection. The research must separate the handler's state-changing setup from its speech-producing branch. Any future implementation remains a normal ClassicSpeech add-on feature: an opt-in, version-guarded, identity-scoped runtime wrapper that preserves the native setup and replaces presentation only. It must restore the original method on add-on termination/reload and fail closed to NVDA-native behavior on any incompatibility; it must not filter final text by words such as `busy`, title text, or document content.
