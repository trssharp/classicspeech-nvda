...
_NATIVE_LABEL = "NVDA native"

_NUMBER_PROCESSING_CHOICES = [
	(_NATIVE_LABEL, "synthesizer"),
	("Single digits", "singleDigits"),
	("Pairs", "pairs"),
...
_SINGLE_DIGITS_THRESHOLD_CHOICES = [
	(_NATIVE_LABEL, "synthesizer"),
	("Five or more digits", "5"),
	("Six or more digits", "6"),
	("Seven or more digits", "7"),
	("Eight or more digits", "8"),
]
...
_CURRENCY_PROCESSING_CHOICES = [
	(_NATIVE_LABEL, "native"),
	("Dollars and cents", "dollarsAndCents"),
]
...
class NumberProcessingPanel(wx.Panel):
	...
	def __init__(self, parent):
		...
		_set_panel_description(
			self,
			"Number Processing",
			"Configure opt-in ClassicSpeech handling for standalone numbers. Currency, dates, times, and phone-like strings are preserved by default.",
		)

		mainSizer = wx.BoxSizer(wx.VERTICAL)
		modeLabel = wx.StaticText(self, label="Number processing:")
		...
		self.numberProcessingMode.SetName("Number processing")
		...
		thresholdLabel = wx.StaticText(self, label="Speak single digits if number contains:")
		...
		self.singleDigitsThreshold.SetName("Speak single digits if number contains")
		...
		phoneLabel = wx.StaticText(self, label="Phone number processing:")
		...
		self.phoneNumberProcessing.SetName("Phone number processing")
		...
		self.friendlyTollFreePrefixes = wx.CheckBox(
			self,
			label="Use friendly toll-free prefixes",
		)
		self.friendlyTollFreePrefixes.SetName("Use friendly toll-free prefixes")
		...
		currencyLabel = wx.StaticText(self, label="Currency processing:")
		...
		self.currencyProcessing.SetName("Currency processing")
		...
		ordinalLabel = wx.StaticText(self, label="Ordinal processing:")
		...
		self.ordinalProcessing.SetName("Ordinal processing")
		...
		dateLabel = wx.StaticText(self, label="Numeric date processing:")
		...
		self.numericDateProcessing.SetName("Numeric date processing")
		...
		dateFormatLabel = wx.StaticText(self, label="Numeric date format:")
		...
		self.numericDateFormat.SetName("Numeric date format")
		...
		self.recognizeIsoDates = wx.CheckBox(
			self,
			label="Recognize ISO year-first dates",
		)
		self.recognizeIsoDates.SetName("Recognize ISO year-first dates")
		...
		self.useWindowsDateFormat = wx.CheckBox(
			self,
			label="Use Windows locale for date format",
		)
		self.useWindowsDateFormat.SetName("Use Windows locale for date format")
		...
		note = wx.StaticText(
			self,
			label=(
				"NVDA native is the safe default and leaves numbers unchanged for NVDA and the active synthesizer. "
